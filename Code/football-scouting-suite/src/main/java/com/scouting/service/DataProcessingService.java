...
/**
 * Questo servizio gestisce il ciclo di vita dei dati all'avvio dell'applicazione. Esegue un processo ETL
 * (Extract, Transform, Load): legge il file CSV generato dallo script Python, pulisce i dati gestendo eventuali
 * valori mancanti o "NaN", e li carica massivamente nel database.
 * <p>
 * È progettato per essere robusto: blocchi try-catch specifici gestiscono errori di parsing su singole righe
 * senza interrompere l'intero processo di caricamento, garantendo che l'applicazione parta anche in presenza
 * di lievi imperfezioni nei dati di input.
 */
...
public class DataProcessingService {
    ...
    public void runPipeline() {
        ...
        try {
            ...
            logger.error("Errore di I/O durante la lettura del file CSV", e);
        } catch (RuntimeException e) {
            logger.error("Errore runtime durante l'elaborazione dei dati", e);
        }
    }
        ...
        for (int i = 0; i < table.rowCount(); i++) {
            try {
             ...
             // Player Info
                ...
                if (++errors <= 5) {
                    logger.warn("Skipping row {}: Dati non validi - {}", i, e.getMessage());
                }
            }
        }
    ...
    private String getString(Table t, int r, String col) {
        ...
        try { 
            ...
            // Fallback se la colonna non è di tipo stringa ma Tablesaw permette la conversione
            Object val = t.column(col).get(r);
    ...
    private Integer getInt(Table t, int r, String col) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Dato non valido nella colonna " + col, e);
        }
    }
